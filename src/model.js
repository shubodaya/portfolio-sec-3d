  ...
  loader.load(
    ...
    (gltf) => {
      ...
      if (modelContainer) {
        ...
        if (hint && containerId === 'gaming-model-container') {
          const hideHint = () => {
            hint.style.display = 'none';
          };
          renderer.domElement.addEventListener('pointerdown', hideHint);
          renderer.domElement.addEventListener('touchstart', hideHint, { passive: true });
          renderer.domElement.addEventListener('mousedown', hideHint);
        }
      }
...
function initModels() {
  const gamingContainer = document.getElementById('gaming-model-container');
  if (gamingContainer && gamingContainer.dataset.modelReady !== '1') {
  ...
  const earthContainer = document.getElementById('model3-container');
...
window.addEventListener('portfolio:route', initModels);
